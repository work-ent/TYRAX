...
const handler = async (m, {conn, args: [effect], text: txt, usedPrefix, command, name}) => {
  if (!effect) throw 'https://whatsapp.com/channel/0029VaNPPwR30LKQk437x51Q*[❗𝐈𝐍𝐅𝐎❗] Correct use of commands sir?*\n—◉ _#logo (efecto) (texto)_\n*EXAMPLE:*\n—◉ _#logo 3d-deep-sea-metal Bumble_\n\n*[❗] When the month says that you have finished, the use would be:*\n—◉ _#logo (efecto) (texto1|texto2)_\n*EXAMPLE:*\n—◉ _#logo Wolf-Logo-Galaxy bumblebee|Bot_\n\n*<𝗟𝗶𝘀𝘁 𝗼𝗳 𝗘𝗳𝗳𝗲𝗰𝘁𝘀/>*\n\n° ඬ⃟📝 #logo ' + effects.map((v) => v.title).join('\n° ඬ⃟📝 #logo ');
  if (!effects.find((v) => (new RegExp(v.title, 'gi')).test(effect))) throw `https://whatsapp.com/channel/0029VaNPPwR30LKQk437x51Q *[❗𝐈𝐍𝐅𝐎❗] The Effect ${effect} 𝗶𝘀 𝗻𝗼𝘁 𝗳𝗼𝘂𝗻𝗱 𝗼𝗻 𝘁𝗵𝗲 𝗹𝗶𝘀𝘁 𝗼𝗳 𝗲𝗳𝗲𝗰𝘁𝘀*`;  
  ...
  const res = await maker(effectoSelect?.url, [...text]).catch(_ => { throw 'https://whatsapp.com/channel/0029VaNPPwR30LKQk437x51Q *[❗] we kijana weka command vizuri mbwa*' })
   if (typeof res == 'number') throw res == -1 ? `https://whatsapp.com/channel/0029VaNPPwR30LKQk437x51Q *[❗𝐈𝐍𝐅𝐎❗] 𝘁𝗵𝗲 𝗲𝗳𝗳𝗲𝗰𝘁 ${effect}  𝗶𝘀 𝗻𝗼𝘁 𝗳𝗼𝘂𝗻𝗱 𝗼𝗻 𝘁𝗵𝗲 𝗹𝗶𝘀𝘁 𝗼𝗳 𝗲𝗳𝗳𝗲𝗰𝘁𝘀*` : `https://whatsapp.com/channel/0029VaNPPwR30LKQk437x51Q *[❗𝐈𝐍𝐅𝐎❗] 𝗧𝗵𝗲 𝗰𝗼𝗿𝗿𝗲𝗰𝘁 𝘂𝘀𝗲 𝗼𝗳 𝗰𝗼𝗺𝗮𝗻𝗱 𝗶𝘀 ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`;
